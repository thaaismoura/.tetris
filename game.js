...
// Permite spawn acima do topo (não conta como colisão)
...
function merge(){
  ...
  for(let y=0;y<matrix.length;y++)
    for(let x=0;x<matrix[y].length;x++)
      if(matrix[y][x])
        state.board[state.pos.y+y][state.pos.x+x]=type;
}
function clearLines(){
  let c=0;
  ...
  if(c){
    const pts=[0,40,100,300,1200][c]*state.level;
    state.score+=pts; state.lines+=c;
...
function hardDrop(){
  while(!collide(state.board,state.piece.matrix,{x:state.pos.x,y:state.pos.y+1}))
    state.pos.y++;
  lockPiece();
}
function move(dx){
...
function drawNext(){
  ...
  const bw=m[0].length, bh=m.length, size=10;
