/* ========= TETRIS – game.js (mobile-first, sem cortes) ========= */

// Dimensões lógicas do tabuleiro (10x20 blocos)
...
function draw(){
  ...
  ctx.fillStyle='rgba(255,255,255,.04)'; for(let y=0;y<ROWS;y++)for(let x=0;x<COLS;x++) ctx.fillRect(x*BLOCK+1,y*BLOCK+1,BLOCK-2,BLOCK-2);
...
function update(t=0){
  ...
  const d=t-state.lastTime; state.lastTime=t; state.acc+=d;
  if(state.acc>=state.dropInterval){ state.acc=0; soft(); }
  draw(); requestAnimationFrame(update);
}

function updateHUD(){ if(scoreEl)scoreEl.textContent=state.score; if(linesEl)linesEl.textContent=state.lines; if(levelEl)levelEl.textContent=state.level; if(speedEl)speedEl.textContent=(1000/state.dropInterval).toFixed(1)+'x'; }
...
function resizeCanvas(){
  ...
  const infoH = getComputedStyle(info).display==='none' ? 0 : (info.getBoundingClientRect().height||0);
  ...
  targetW = Math.max(210, Math.min(330, targetW));   // limites seguros
...
if(btnAgain) btnAgain.addEventListener('click', ()=>{ start(); }); // começa direto novamente

window.addEventListener('load', ()=>{
  if(overlayStart) overlayStart.style.display='flex';
  draw();           // grade inicial
  bindResize();     // garante que nada corte
});
